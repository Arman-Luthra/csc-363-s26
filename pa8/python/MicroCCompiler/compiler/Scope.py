...
class Scope(ABC):
    ...
    class SymbolTableEntry(object):
        ...
        def __str__(self) -> str:
            return f"; name {self.getName()} type {self.getType().name} location {'0x{:08x}'.format(self.getAddress())}"
    ...
    def checkSymbol(self, name: str) -> ErrorType:
        if not self.searchLocalScope(name) is None:
...
class LocalScope(Scope):
    ...
    startingArgsOffset: int = 8 # start argument offset with room for old return address and return value
